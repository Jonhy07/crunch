...
class HomologationModelForm(forms.ModelForm):
	...
	# item2 = ModelChoiceField(queryset=Item.objects.filter(),  empty_label='Please select a item')
	# item1 = ModelChoiceField(queryset=Item.objects.filter(),  empty_label='Please select an item')
	item2 = ModelChoiceField(queryset=Item.objects.filter(),  empty_label='Please select an item')
	class Meta:
		model = Homologation
		fields = ['item1','item2']